...
|---------------|----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|------------------------------------------------------------------------|------------------------------------------------------------------------|------------------------------------------------------------------------|
| **sort**      | `numbers.sort` คืนค่า array ใหม่ โดยไม่เปลี่ยนค่าเดิม                | ใช้ `qsort` กับ array ที่คัดลอกมาจากของเดิม <br>(ผ่าน `malloc + memcpy`)                                     | ใช้ `stream().sorted()` สร้าง List ใหม่                              | ใช้ `sorted(list)` คืนค่า list ใหม่                                    | - ไม่กระทบข้อมูลเดิม<br>- ปลอดภัย                                     | - ต้องมี array/list ใหม่เพิ่มขึ้นมา<br>- กินหน่วยความจำมากกว่าเล็กน้อย |
...
| **sort_by**   | `words.sort_by { |w| w.length }` คืนค่า array ใหม่ โดยไม่เปลี่ยนค่าเดิม | ใช้ `qsort` กับ array ที่คัดลอกมา <br>และใช้ฟังก์ชัน `compareByLength`                                      | ใช้ `stream().sorted(Comparator.comparingInt(...))` สร้าง List ใหม่ | ใช้ `sorted(list, key=...)` คืนค่า list ใหม่                           | - เลือกเงื่อนไขการเรียงได้ยืดหยุ่น<br>- ข้อมูลเดิมไม่ถูกแก้ไข          | - ต้องมี array/list ใหม่<br>- กินหน่วยความจำเพิ่ม                      |
| **sort_by!**  | `words.sort_by! { |w| w.length }` เรียงใน array เดิม                 | ใช้ `qsort` โดยตรงกับ array เดิม <br>และกำหนด comparator เอง                                                 | ใช้ `Arrays.sort(array, Comparator.comparingInt(...))`               | ใช้ `list.sort(key=...)` เรียงใน list เดิม                             | - ยืดหยุ่น (กำหนดเงื่อนไขได้)<br>- ไม่ต้องสร้าง array ใหม่             | - ข้อมูลเดิมถูกเปลี่ยน<br>- อาจเสี่ยงหากต้องการเก็บค่าต้นฉบับไว้ด้วย |

---

## ✅ สรุปสั้น ๆ
- ถ้า **ต้องการเก็บข้อมูลต้นฉบับ** → ใช้ `sort` หรือ `sort_by` (ทุกภาษา)  
- ถ้า **เน้นประสิทธิภาพและหน่วยความจำ** → ใช้ `sort!` หรือ `sort_by!` (ทุกภาษา)  
- ภาษา C ต้องสร้าง comparator เอง (ยืดหยุ่นมาก แต่โค้ดยุ่งกว่า)  
- Ruby / Python ใช้ง่ายที่สุด, Java อยู่กึ่งกลาง, C ยากที่สุดแต่ควบคุมละเอียดได้มาก  
