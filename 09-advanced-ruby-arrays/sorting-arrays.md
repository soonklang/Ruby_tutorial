...
โดยการทำแบบนี้จะเป็นการเรียงลำดับค่าใน Array โดยจะต้องมี Array อีกตัวหนึ่งในการเก็บค่า Array อีกตัวที่ได้ทำการเรียงข้อมูลผ่านการ sort ไว้แล้ว โดยวิธีการใช้งานสามารถทำได้ดังนี้ :
...
โดยการทำแบบนี้จะเป็นการเรียงค่าใน Array โดยสามารถเรียงค่าผ่านทาง Array ได้โดยตรง ผ่านการใช้ sort! ได้เลย ซึ่งจะแตกต่างจาก sort ตรงที่จะต้องมีตัว Array อีกตัวที่คอยมารับค่า โดยวิธีการใช้งานสามารถทำได้ดังนี้ :
...
โดยการทำแบบนี้จะเป็นการเรียงค่าใน Array โดยใช้ Block หรือเรียกอีกอย่างว่าเงื่อนไข ในการเรียงค่า โดยจะต้องมี Array อีกตัวที่คอยรับค่าที่ผ่านการเรียงข้อมูลด้วย sort\_by ไว้แล้ว โดยวิธีการใช้งานสามารถทำได้ดังนี้ :
...
โดยการทำแบบนี้จะเป็นการเรียงค่าใน Array โดยใช้ block หรือเรียกอีกอย่างว่าเงื่อนไข โดยสามารถเรียงค่าผ่านทาง Array ได้โดยตรงผ่านการใช้ sort\_by! ได้เลย ซึ่งจะแตกต่างจาก sort\_by ตรงที่จะต้องมีตัว Array อีกตัวที่คอยมารับค่า โดยวิธีการใช้งานสามารถทำได้ดังนี้ :
...
| ------------- | ----------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **sort**      | `numbers.sort` คืนค่า array ใหม่ โดยไม่เปลี่ยนค่าเดิม | ใช้ `qsort` กับ array ที่คัดลอกมาจากของเดิม (ผ่าน `malloc + memcpy`) | ใช้ `stream().sorted()` สร้าง List ใหม่ | ใช้ `sorted(list)` คืนค่า list ใหม่                                 | <p>- ไม่กระทบข้อมูลเดิม<br>- ปลอดภัย</p>                            | <p>- ต้องมี array/list ใหม่เพิ่มขึ้นมา<br>- กินหน่วยความจำมากกว่าเดิมเล็กน้อย</p> |
| **sort!**     | `numbers.sort!` เรียงใน array เดิม                    | ใช้ `qsort` กับ array เดิมโดยตรง                                     | ใช้ `Arrays.sort()` เรียงใน array เดิม  | ใช้ `list.sort()` เรียงใน list เดิม                                 | <p>- ประหยัดหน่วยความจำ<br>- ไม่ต้องสร้าง array ใหม่</p>            | <p>- ทำให้ข้อมูลเดิมหายไป<br>- อาจผิดพลาดถ้าเผลอแก้ไขข้อมูลต้นฉบับ</p>            |
| **sort\_by**  | words.sort\_by {เงื่อนไข}                             | ไม่มีตัวสนับสนุนต้องเขียนขึ้นมาเอง                                   | คืนค่า array ใหม่ โดยไม่เปลี่ยนค่าเดิม  | ใช้ `qsort` กับ array ที่คัดลอกมา แล้วใช้ฟังก์ชัน `compareByLength` | ใช้ `stream().sorted(Comparator.comparingInt(...))` สร้าง List ใหม่ | ใช้ `sorted(list, key=...)` คืนค่า list ใหม่                                      |
| **sort\_by!** | words.sort\_by! {เงื่อนไข}                            | ไม่มีตัวสนับสนุนต้องเขียนขึ้นมาเอง                                   | เรียงใน array เดิม                      | ใช้ `qsort` โดยตรงกับ array เดิม และกำหนด comparator เอง            | ใช้ `Arrays.sort(array, Comparator.comparingInt(...))`              | ใช้ `list.sort(key=...)` เรียงใน list เดิม                                        |
