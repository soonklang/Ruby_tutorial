...
| **sort_by**   | words.sort_by {เงื่อนไข}|ไม่มีตัวสนับสนุนต้องเขียนขึ้นมาเอง| คืนค่า array ใหม่ โดยไม่เปลี่ยนค่าเดิม | ใช้ `qsort` กับ array ที่คัดลอกมา แล้วใช้ฟังก์ชัน `compareByLength`               | ใช้ `stream().sorted(Comparator.comparingInt(...))` สร้าง List ใหม่ <br>ใช้ `sorted(list, key=...)` คืนค่า list ใหม่                           | - เลือกเงื่อนไขการเรียงได้ยืดหยุ่น<br>- ข้อมูลเดิมไม่ถูกแก้ไข          | - ต้องมี array/list ใหม่<br>- กินหน่วยความจำเพิ่ม                      |
| **sort_by!**  | words.sort_by! {เงื่อนไข}|ไม่มีตัวสนับสนุนต้องเขียนขึ้นมาเอง|เรียงใน array เดิม                 | ใช้ `qsort` โดยตรงกับ array เดิม และกำหนด comparator เอง                          | ใช้ `Arrays.sort(array, Comparator.comparingInt(...))`<br>ใช้ `list.sort(key=...)` เรียงใน list เดิม                             | - ยืดหยุ่น (กำหนดเงื่อนไขได้)<br>- ไม่ต้องสร้าง array ใหม่             | - ข้อมูลเดิมถูกเปลี่ยน<br>- อาจเสี่ยงหากต้องการเก็บค่าต้นฉบับไว้ด้วย |
